...
const StyledTd = ({children})=><td className="text-base text-[var(--text-primary)] px-2 py-1 font-semibold border-r-1 border-r-[var(---text-secondary)]">{children}</td>
...
const VendorOrdersTable = ({ordersData})=>{
	...
	{console.log(ordersData)}
        <table className="border-collapse border-2 border-[var(---text-secondary)] rounded-md">
            ...
            <tbody>
                {ordersData.map(({orderId, name, customerContactInfo: {phone}, createdAt, quantityOrdered, deliveryStatus, price})=>{
                    return (
                        <tr className="border-b-1 border-b-[var(---text-secondary)]" onClick={()=>router.push(`/${userId}/vendor/orders/${orderId}`)}>
                            ...
                            <StyledTd onClick={()=>router.push(`/${userId}/vendor/orders/${orderId}`)}><span className="flex items-center gap-1"><FaEye /> View</span></StyledTd>
                        </tr>
                    )
                })}
            </tbody>
